...
sub usage {
    ...
    print "  -ents file	      Read and create files only for the entities with the unique name listed in file.\n";
...
sub parse_args {
    ...
    $kinds = "function, procedure, subroutine, method" unless $kinds;
...
sub export_graphics {
	...
	foreach my $funcname (<INFILE>) {
	    $funcname =~ s/^\s*//;
	    $funcname =~ s/\s*$//;
	    push @functions, $db->lookup_uniquename($funcname);
	}
	close INFILE;
    } else {
